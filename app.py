...
import threading
import subprocess
import urllib.request  # 👑 補上缺少的標準庫匯入
import base64          # 👑 補上缺少的標準庫匯入
...
# 👑 4. 雲端清除衝突節點與5輪閉環自癒（修正縮排、密鑰格式與執行順序）
def tailscale_smart_autofix_and_up():
    ...
    # 🚀 第一次登入嘗試（如果正統名字被佔用，此時本機名字會被降級為 -1 或 -2）
    ...
    for big_loop in range(1, 6):
        ...
        try:
            # 查戶口：路徑統一使用 /usr/local/bin/tailscale
            ...
            # 【終極目標】名字完全歸位，大獲全勝，直接退出自癒
            if full_status_name == "render-proxy":
                print(f"🎉🎉🎉 【大獲全勝】本機名稱已完美鎖定為 [render-proxy]！", flush=True)
                break
                
            # 如果發現自身名字帶有髒後綴（例如 render-proxy-1, render-proxy-2）
            if "render-proxy" in full_status_name and full_status_name != "render-proxy":
                ...
                if target_api_id:
                    ...
                    # 🔥 核心修正：金鑰放前面作為用戶名，格式為 api_secret:
            ...
            print(f"❌ [智能自愈] 自愈大循環執行異常: {str(e)}", flush=True)
...
def master_orchestrator():
    ...
    with recovery_lock:
        ...
        launch_tailscaled()                 # Step 2: 本地點火（先拉起守護進程，後面才能查狀態）
        tailscale_smart_autofix_and_up()   # Step 3: 進入 5 輪自癒奪名大循環
        launch_gost()                      # Step 4: 網路通道完全歸位後，拉起轉發橋樑
